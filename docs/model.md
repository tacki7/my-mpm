...
前後方張力は板の端面にかかる応力として与える。端の列の粒子に、応力 × 粒子の**現在の**高さ（|F·e_y|·dp）の力をかけるので、
合計は応力 × その時点の板厚になる（圧延後の頭端では σf·h1。初めの実装は未変形の高さで掛けていて、出側の板に σf/(1−r) が掛かっていた）。

- 後方張力: 計算の開始から立ち上げ、尾端が入口面（x = −Lc）に達したら立ち下げる（ロールの後ろの板が無くなれば引く相手も無い。
  以前は抜けた板を引き戻していた）
- 前方張力: 頭端が出側の測定位置を越えたとき（巻取りに掴まれたとき）から立ち上げる
- 立ち上げ・立ち下げは `rolling.tensionRamp`（既定: 弾性波が板の長さを 10 回伝わる時間 10·L/c。質量スケーリング 1e4・
  L = 16 mm で約 2.7 ms）。階段状に掛けると応力波が鳴り、それだけで頭端が割れることがあった
- 前方張力と後方張力の差をロールの摩擦で支えられないと、板はロールより速く滑る（スキッド）。先進率が数十 % を超えたらこれを疑う
