...
（約 12 分、うちタンデムの節が約 9 分。実クリックでタブを移り、板幅 4 mm を最後まで回して `node tools/solid.mjs` と比べる（相対 1e-5）、色の量のタブ、実ドラッグ・ホイール・ダブルクリック、見る向き、URL、
...
`solid`（3 次元のタブ: `active` `ready` `running` `frames` `done` `diag`（`steady` が定常の平均、SI）`geometry`（今のスタンドの。`stand` `stands` `sheetLength` も）`stand` `stands` `standResults`（済んだスタンドの `Stand3Result`）`stopped` `settings` `params` `field` `range` `url` `view`（向き・拡大・切る・ロール）`tracks`（追っている点: 最初の亀裂・損傷最大）`explorer`（表示中の点）`setDim('2'|'3')` `setField()` `run()` `drawMs()`）
