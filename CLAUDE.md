...
`&field=seq|eta|s1|pres|ep|damage|sxx|syy|sxy|dT|lagrange|porosity|loc|drucker|rate&autorun=1&stopafter=<step>`
...
`&handoff=done|steady|crop`（タンデムの引き継ぎ。既定は `done` = 板が抜けてから。`steady` = 定常になったらすぐ次のスタンドへ、定常の部分を繰り返した板で。数倍速い。`crop` = 板の中央の、次のスタンドで定常になるのに要る長さだけを、それがロールを抜けたら。`phase: 'cropped'`）
