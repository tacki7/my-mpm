// The plan view's URL keys (W, wcells, notch) as the page reads them (src/app/planQuery.ts): each in
// its range or ignored, parsed like the section model's keys, checked together — a shared link must
// not start more points than MAX_POINTS (as applyQuery does for the section model), and a notch deeper
// than a quarter of the width is ignored — and written back to the same settings.
// @check
...
const same = (a, b) => a.width === b.width && a.cells === b.cells && a.notch === b.notch;
...
for (const qs of ['', 'W=16&wcells=8&notch=0.5', 'W=37.5&wcells=13', 'notch=2.25']) {
  const a = of(qs);
