...
const surface = () => {
  for (let i = 0; i < NI; i++) {
    for (const [k, j] of [[0, NJ - 1], [1, 0]]) {
      ...
      // the edge along the roll normal: half the deformed y edge projected on n (|F e_y| itself grows with shear)
      const sink = (roll.R + 0.5 * dp * Math.abs((rx * sim.f01[q] + ry * sim.f11[q]) / d) - d) / dp;
...
while (sim.step < 60000) {
  ...
  if (d.phase === 'steady') {
    ...
    hill.push({ F, saw: jump / sum, repeated, xn: d.neutralX, state: d.neutralState, ford: -Math.sqrt(Math.max(0, d.forwardSlip ?? 0) * P.rolling.rollRadius * (d.exitThickness ?? NaN)) });
...
ok(hill.every((h) => h.state === 'found' && h.xn > -sim.contactLength && h.xn < 0), 'neutral point found, inside the bite', hill.map((h) => (h.xn == null ? h.state : (h.xn * 1e3).toFixed(3) + ' mm')).join(', '));
...
between(sinkMax, 0, 0.02, 'no particle edge sinks into a roll (deepest edge along the roll normal, in dp; 0.07 with the roll-side nodes held to the roll surface velocity alone)');
