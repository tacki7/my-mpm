...
4.31 kN/mm、10 セルで 3.56 kN/mm で、スラブ法からの超過は 6 → 10 → 14 → 20 セルで 42 → 18 → 15 → 11 % とまだ収束していない
（[docs/validation.md](docs/validation.md)「標準条件」「スラブ法との比較」）。条件どうしを比べるときは格子を揃える。
...
- **圧延荷重**（ロール 1 本あたり、板幅 1 mm あたり）: 時間の推移。太い線は移動平均（窓は格子をまたぐ揺れの周期 2h/v_in の
  ...
  格子が粗いと MPM の荷重はスラブ法より高く出る（標準条件で 6 セル 4.31・10 セル 3.56 対 3.03 kN/mm。上の「格子で結果が変わる」）。
  移動平均は描画だけで、結果の表と書き出しの CSV は元の値
- **接触面の圧力と摩擦応力（フリクションヒル）**: 入口から出口までの分布と中立点。スラブ法の p（破線）・τ（点線）と中立点
  （白抜きの丸）を重ねる。張力が変形抵抗 2k に達する・摩擦が固着になる・中立点が無い条件ではスラブ法が成り立たないので重ねず、
  凡例に理由を出す
...
数値を読む前に [docs/model.md](docs/model.md)「既知の限界」を見る。主なもの: 2D 平面ひずみで板幅方向が無いので
耳割れは扱えない。ロールは剛体（偏平しない）。計算を速めるために質量スケーリング（既定 1e4 倍）を使っている。
