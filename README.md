...
- 条件の欄の「条件の振り方（比較）」で、**板厚（母板）・板幅・ロール径・摩擦係数**のうち振るものにチェックを入れ、最初と最後の値を入れる
  （いくつ選んでも同時に、等間隔に変わる）。「条件の数」（2〜20、既定 10）、「パス数」（タンデムのスタンド数、既定 4）、「スタンドの引き継ぎ」、「同時に回す数」
  （1 条件に 1 スレッド。既定は論理コア数 − 1、8 まで）。振らない量と、セル数・入側の板クラウン・ロールの撓みは「板と格子（3 次元）」「ロールの撓み（3 次元）」の欄、
  材料・圧下率・張力・ロール偏平は共通の欄の値。板の長さは条件ごとに「定常状態になるまで」
- 「比較を始める」で回す。条件ごとの進み具合が上の帯に出て、終わった条件から図と表に入る。一時停止・続けるは全部の条件に効き、タブを移ると一時停止する
...
- URL: `?dim=c&W3=4&cells3=4&sv=h0:0.8:1.2,W:4:8&sn=10&sp=4&sh=steady&sj=4`（`sv` = 振る量と範囲（`h0`・`W`・`D` は mm、`mu`）、`sn` 条件の数、`sp` パス数、
  `sh` 引き継ぎ（`steady`・`crop`・`done`）、`sj` 同時に回す数）。`&autorun=1` で開いてすぐ回す
- ヘッドレス: `node tools/sweep.mjs --W 4 --vary h0=0.8:1.2 --n 10 --stands 4 [--vary D=150:250] [--handoff crop] [--jobs 4] [--json]`（ほかの条件は `tools/solid.mjs` と同じ引数）
