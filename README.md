...
| 前方張力が過大 | 出口の先で板が伸び、くびれて破断する | 既定の前方張力 480 MPa では割れない。6 セルなら 550 MPa から割れる |
| 厚肉・軽圧下の中心割れ | 変形が板厚中心に届かず、中心に静水圧の引張 | 未調整 |
| 内部欠陥（空洞）起点 | 空洞がバイトで潰れるか、出口側で開くか | 未調整 |
| 高摩擦・低延性 | 表層から割れる | 未調整 |

「未調整」は値を当て推量で入れただけで、見せたい現象がまだ出ないことがある。
...
4.30 kN/mm、10 セルで 3.56 kN/mm で、スラブ法からの超過は 6 → 10 → 14 → 20 セルで 42 → 18 → 15 → 11 % とまだ収束していない
（[docs/validation.md](docs/validation.md)「標準条件」「スラブ法との比較」）。条件どうしを比べるときは格子を揃える。

条件は URL でも渡せる（例 `?preset=front-tension&tf=600&cells=6&L=8&field=damage&autorun=1`。長さは mm、張力は MPa、
...
<!-- 三軸度の絵: 体積の扱いの作り直し（T21）の後に、下の URL で撮り直して差し替える -->
![薄板の標準圧延の応力三軸度 η（T21 前の値）](docs/img/standard-eta.png)

条件 `?preset=standard&cells=8&L=10&field=eta&autorun=1&stopafter=13000`。
**この絵は体積の扱い（J-bar）を作り直す前の値。** 今のソルバーは平面ひずみの板幅方向 σzz が整合していない
（[docs/model.md](docs/model.md)「分岐の指標」）ので、η・静水圧 p・板厚中心の応力は色の傾向を見るだけにして、
定量的な結論には使わない。
...
![前方張力が過大（前方張力 600 MPa）で出た亀裂と損傷 D](docs/img/front-tension-crack.png)

条件 `?preset=front-tension&tf=600&cells=6&L=8&field=damage&autorun=1`、最初の亀裂が出たところで一時停止。
最初の亀裂は 13.26 ms（[docs/validation.md](docs/validation.md)「非局所化」の 6 セル・粒子ごとの行と同じ条件）。
...
- **圧延荷重**（ロール 1 本あたり、板幅 1 mm あたり）: 時間の推移
- **接触面の圧力と摩擦応力（フリクションヒル）**: 入口から出口までの分布と中立点
...
亀裂はきれいに開かない。さらに、体積の扱い（J-bar）は作り直す予定で、それまで η・静水圧 p・板厚中心の応力・
板幅方向 σzz は定量的な結論に使わない。検証の値と測定条件は [docs/validation.md](docs/validation.md)。
