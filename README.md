...
- **板とロール**: 入側板厚・板厚方向（全厚・対称。下の「板厚方向の対称モデル」）・圧下率・ロール半径・板の長さ（「板の長さの取り方」を「定常状態になるまで」にすると、定常になって定常の読みが揃うのに要る長さを自動で決める。ロール偏平・圧下率一定ではロールの調整が落ち着くぶん長い。決めた長さは圧延を始めると「板の長さ」に出る。`&length=steady`、ヘッドレスは `--length steady`）
...
| ひずみ速度 | 相当ひずみ速度 [1/s]（実機の速度 `millSpeed` に換算） | ロールバイトの中だけ大きく、入口で最大。流動応力と JC の破断ひずみが見る値。3 次元のタブにもある |
...
条件パネルの「板とロール」の **「板厚方向」**（URL は `sym=1`、ヘッドレスは `--half`）。「対称（上半分・ロール 1 本）」にすると、板厚の中心
y = 0 を対称面にして上半分だけを解く（3 次元の 1/4 モデルと同じ）。点が半分なので約 2 倍速い（6 セル・板長 8 mm で 6.0 → 3.0 s）。
荷重・トルク・出側板厚・先進率・塑性仕事は板全体の値（ロール 1 本あたり）で、偶数セルなら全厚の計算と丸めの範囲で同じ
（荷重の相対差 6e-13。[docs/validation.md](docs/validation.md)「板厚方向の対称モデル」）。絵は下半分を鏡で写して描き、y = 0 に点線の継ぎ目。
下半分の点をクリックすると鏡の元の点（上半分）が選ばれる。中心割れは対称面の上の行から始まり、「場を分ける」なら面が対称面の上で開く
（面の扱いは全厚と同じ数にはならない。[docs/model.md](docs/model.md)「板厚方向の対称モデル（2 次元）」）。タンデムでも使える。
平面図と 3 次元には効かない。既定は全厚（ロール 2 本）で、既定の計算は変わらない。
...
時計の下の「描画の更新」で、ワーカーが絵を送る間隔を選ぶ（自動 = 断面・平面図 30 回/秒、3 次元 12 回/秒。30 回/秒〜5 秒に 1 回）。
計算中に変えると次の絵から効き、断面・平面図・3 次元のどれにも効く。少なくすると描画に取られる分だけ計算が進む（細かい格子や広い板で）。
選択はブラウザに保存され、条件の URL には入らない。`__mpm.frameMs`（読み書き。0 が自動）。

### レイアウト

タブの行の右端の「レイアウト」で、標準とコンパクトを選ぶ。コンパクトは字を一段小さく、余白を詰め、条件の欄の説明文と表題の副題を隠し、グラフを低くする
（小さな画面や、ほかの窓と並べて見るとき）。計算と数値には影響しない。選択はブラウザに保存され、条件の URL には入らない。`__mpm.density`（読み書き。'standard' | 'compact'）。
...
「中央部を切り出す（次の定常に要る長さ）」（URL は `&handoff=crop`）にすると、板が長くなりすぎないよう、板の長手の中央から、次のスタンドで
定常になるのに要る長さだけを切り出して次のスタンドへ渡す。切り出す部分がロールを抜けたところで次を始める（尾端は圧延しない）。
3 スタンド・6 セル・板長 16 mm で板全部 376 s → 100 s、荷重は 0.4 % 以内。定常の部分を繰り返す「定常になったらすぐ」と違い、板の長手の変化はそのまま渡る。
...
- 条件の欄の「板と格子（3 次元）」で板幅（2〜200 mm。時間は板幅に比例し、40 mm で約 14 分）・板の長さ・板厚方向のセル数（4・6・8）を決める。板厚・圧下率・ロール半径・摩擦・材料・
  ...
  2 次元と同じ欄で決める。前後の張力（「潤滑と張力」の欄）も効く: 後方張力は最初から、前方張力は頭端が出てから立ち上げ、定常はそれが乗り切るのを待つ。
  結果の表に「後方張力 / 前方張力（今 / 条件）」の行が出る（尾端がロールに来ると後方張力は 0 になる）。GTN・亀裂の面は 3 次元には無い
- タンデムは、出てきた板の板厚と**板幅**（幅広がりのぶん広い）と**板の形**（断面のクラウン・端のふくらみ、板が抜けてから・切り出しでは平面形と長手の変化も）を次のスタンドの入側にして、応力・ひずみ・損傷を引き継ぐ。右の欄に「スタンドごとの結果（3 次元）」が出る。
  後のスタンドほど格子が細かく重いので、引き継ぎは「定常になったらすぐ」か「中央部を切り出す」が速い（W 4 mm・2 スタンドで 603 s → 305 s）
...
- **巻き戻して再生**: 計算が止まると（終わっても、一時停止でも）絵の下に「巻き戻す」「再生」とスライダーが出て、記録した絵を最初から見直せる。色の量のタブ・向き・拡大はそのまま効き、時計と荷重のグラフの印（「再生」）がその絵の時刻を指す。記録は数百枚まで（長い計算は等間隔に間引く）。**「動画に保存」**で記録した絵を今の向き・色の量・速さで動画ファイルに（幅 1280 px、凡例と時刻入り。ブラウザの中で H.264 の MP4 に、できないブラウザでは VP9 の WebM に。コンテナは `src/app/mux.ts` で自前に書く）。スライダーの右端が今の絵で、「続ける」「やり直す」でも今の絵に戻る
...
- 「板厚の全体を解く（ロール 2 本）」にすると板厚の中央を対称面にせず、上下のロールの両方と板厚の全体を解く（板幅の中央の対称はそのまま）。点が 2 倍で時間も 2 倍。
  上下対称な条件では 1/4 モデルと同じ答え（`docs/validation.md`）。URL は `full3=1`、ヘッドレスは `node tools/solid.mjs --full`
- **ロールの撓み**（「ロールの撓み（3 次元）」の欄）: 「ロールが荷重で撓む」にすると、ロールを梁（直径 2R の丸棒、曲げ + せん断）として、板から受ける荷重の
  幅方向の分布で毎ステップ撓みを解き、ロールの位置に足す（荷重と連立）。**バレル長**（胴の長さ、板幅の 1.5 倍以上）と、**支点**をバレルの端か軸受（支点間距離を入力。
  胴の外に張り出したぶん撓みが増える）から選ぶ。板は胴の中央。下のロールも同じだけ撓むので、板は撓みの 2 倍厚く出て、中央が厚い（クラウン）。
  結果の表に「ロールの撓み（板幅の中央 / 板の端）」「クラウン」の行が出る（落ち着くまでは仮の値）。ヤング率は「ロール偏平」のロールのヤング率。
  R 100 mm・板幅 8 mm ではおよそ 1 µm で、細いロールや広い板で効く。初期クラウン・バックアップロールはまだ無い
- **入側の板クラウン**（「板と格子（3 次元）」の「入側の板クラウン」、µm）: 入側の板厚を幅方向に 2 次曲線で与える（板幅の中央が h0、端が h0 − クラウン。負なら中央が薄い）。
  下のグラフに**板クラウン**（入側・出側、定常。板厚 − 端の板厚）と**平坦度**（出側、定常、長手方向。形状計の読みで + が伸び、I 単位 = 10⁻⁵）の図が加わり、
...
- **CPU のコア数**（同じ欄の「CPU のコア数」、1〜その機械の論理コア数。URL は `threads3=N`）: 1 ステップを N 本のスレッド（Worker）で板を圧延方向に分けて解く。
  結果は 1 本のときと同じ（和の順序の丸めだけ）。板幅 8 mm・4 セルで 4 本にすると 2.4 倍ほど（M2）。`SharedArrayBuffer` が要るので dev / preview サーバー（COOP / COEP
  のヘッダ付き）で効き、付かないところでは 1 本で計算して選択の下にそう出す。GPU のときは使わない。ヘッドレスは `node tools/solid.mjs --threads 4`（`docs/model.md`「CPU の複数スレッド」）
...
### 条件の比較（母板の条件を振って、何パスか後の結果を並べる）

表題の下の「条件の比較」のタブで、3 次元のタンデムを条件を変えながら何本も回し、最後のパスのあとの結果を並べて比べる（`src/app/sweepMode.ts`、`src/mpm/solid/sweep.ts`）。

- 条件の欄の「条件の振り方（比較）」で、**板厚（母板）・板幅・ロール径・摩擦係数**のうち振るものにチェックを入れ、最初と最後の値を入れる
  （いくつ選んでも同時に、等間隔に変わる）。「条件の数」（2〜20、既定 10）、「パス数」（タンデムのスタンド数、既定 4）、「スタンドの引き継ぎ」、「CPU のコア数」
  （条件は 1 つずつ順に回し、その 1 条件を何本のスレッドで計算するか。3 次元のタブの「CPU のコア数」と同じ仕組み。既定は 4、論理コア数 − 1 まで）。振らない量と、セル数・入側の板クラウン・ロールの撓みは「板と格子（3 次元）」「ロールの撓み（3 次元）」の欄、
  材料・圧下率・張力・ロール偏平は共通の欄の値。板の長さは条件ごとに「定常状態になるまで」
- 「比較を始める」で回す。条件ごとの進み具合が上の帯に出て、終わった条件から図と表に入る。一時停止・続けるは計算中の条件に効き、タブを移ると一時停止する
- 図は 6 つ: **板クラウン**（最後のパスの出側）・**圧延荷重**（パスごとに色を替えて）・**幅広がり**（全パスで W / W0 − 1）を振った量に対して、
  **出側の板厚の分布**（最後のパス、条件ごとの色を重ねて）、**平坦度**（中央 − 端）、**出側の板幅**（パスごと）。振った量が 1 つならその値が横軸、2 つ以上なら条件の番号。
  下の表に条件ごとの数値（CSV に保存できる）。「条件の URL をコピー」で同じ振り方の比較のタブが開く
- URL: `?dim=c&W3=4&cells3=4&sv=h0:0.8:1.2,W:4:8&sn=10&sp=4&sh=steady&st=4`（`sv` = 振る量と範囲（`h0`・`W`・`D` は mm、`mu`）、`sn` 条件の数、`sp` パス数、
  `sh` 引き継ぎ（`steady`・`crop`・`done`）、`st` 1 条件のスレッド数）。`&autorun=1` で開いてすぐ回す
- ヘッドレス: `node tools/sweep.mjs --W 4 --vary h0=0.8:1.2 --n 10 --stands 4 [--vary D=150:250] [--handoff crop] [--jobs 4] [--json]`（ほかの条件は `tools/solid.mjs` と同じ引数。ツールは `--jobs` の数の条件を 1 スレッドずつ並べて回す）
